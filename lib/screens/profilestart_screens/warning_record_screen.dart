...
class _WarningRecordScreenState extends State<WarningRecordScreen> {
  ...
  Future<void> fetchWarnings() async {
    final response = await http.post(
      Uri.parse('http://10.0.2.2:3000/get_warning'),
  ...
  String getDurationForAxis(String axis, int index) {
    ...
    for (int i = index + 1; i < warnings.length; i++) {
      if (warnings[i]['axis'] == axis && warnings[i]['level'] != warnings[index]['level']) {
        DateTime endTimestamp = DateTime.parse(warnings[i]['timestamp']);
        return calculateDuration(startTimestamp.toIso8601String(), endTimestamp.toIso8601String());
      }
    }
    return '';
  }
      ...
      body: Padding(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            String duration = '';
            for (int i = index + 1; i < warnings.length; i++) {
              if (warnings[i]['axis'] == warning['axis'] && warnings[i]['level'] != warning['level']) {
                duration = calculateDuration(warning['timestamp'], warnings[i]['timestamp']);
                break;
              }
            }
