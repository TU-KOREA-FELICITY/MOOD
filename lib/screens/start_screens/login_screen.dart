...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _login() async {
    ...
    try {
      await http.post(Uri.parse('http://192.168.112.219:3000/login'));
      _checkAuthStatus();
    } catch (e) {
      setState(() {
        _status = '로그인 오류';
  ...
  void _checkAuthStatus() async {
    try {
      final response =
      await http.get(Uri.parse('http://192.168.112.219:3000/check_auth'));
  ...
  Future<Map<String, dynamic>> _loginComplete(String userAwsId) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.112.219:3000/login_complete'),
  ...
  Future<void> _saveUserInfoAndNavigate() async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.112.219:3000/register_complete'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
          'user_aws_id': 'sun2',
          'username': '장인선',
