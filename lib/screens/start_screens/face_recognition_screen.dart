...
class _FaceRecognitionScreenState extends State<FaceRecognitionScreen> {
  ...
  Future<Map<String, dynamic>> _checkIdDuplicate(String userAwsId) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.112.219:3000/check_id_duplicate'),
  ...
  void _register(String username) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.112.219:3000/register'),
        headers: {'Content-Type': 'application/json'},
  ...
  void _checkRegistrationStatus(String username) async {
    try {
      final response =
          await http.get(Uri.parse('http://192.168.112.219:3000/check_registration'));
      final result = json.decode(response.body);
