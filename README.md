...
< 기능 >​
- 얼굴 기울기(Pitch(상하), Roll(좌우)), 양쪽 눈 높이를 고려한 집중도 측정​
- 설정 임계값 이하로 3초 동안 집중도가 낮아지면 레벨에 따른 알림을 제공​
- 경고 단계: 경고 -> 주의 -> 위험
<br>

< 다루는 정보 >​
- 웹캠 프레임: 실시간 영상 획득 및 분석​
- 머리, 눈 움직임: 얼굴 방향(Pitch, Roll)과 양쪽 눈 높이 변화량 저장 및 분석​
<br>

< 시점 >​
- 집중도 인식/분석: 실시간​
- 경고 발생: 임계값 이하 3초 지속 시​
- 경고 해제: 집중도가 일정 기준 이상으로 복구될 때 
<br>
...
< 적용 알고리즘 >​
- 실시간 영상 스트리밍: OpenCV를 활용한 영상 프레임 처리
- 얼굴 기울기 추적​
...
< 기능 >​
- 등록 : 입력받은 ID와 회원 정보를 바탕으로 회원가입​
- 인증: AWS Rekognition 얼굴 비교 결과로 반환된 ID를 이용해 로그인​
<br>

< 다루는 정보 >​
- 입력받은 ID : 사용자 구분을 위한 텍스트 정보​
- 회원정보 : 사용자 이름, 차종, 선호 취향(장르, 아티스트)​
- 반환된 ID : Amazon S3 버킷에 등록된 ID(사용자 등록 시 입력받은 ID)​
<br>
...
< 기능 >​
- 감정 분석 함수 호출 시 감정별 신뢰도를 출력​
- 감정 신뢰도 기준으로 내림차순 정렬 후 상위 3가지 감정 저장​
<br>

< 다루는 정보 >​
- user_id: 운전자 고유 식별자​
- emotions: 상위 3가지 감정 텍스트
<br>
...
< 기능 >​
- 얼굴 기울기(Pitch, Roll) 및 양쪽 눈 y좌표 움직임 기반으로 집중도 측정​
- 실시간 분석하여 기준 이하 집중도 발생 시 경고 제공
<br>

< 다루는 정보 >​
- user_id: 운전자 고유 식별자​
- axis: 얼굴 기울기 정보 (Pitch, Roll)​
- focus_level: 집중도 레벨
<br>
...
< 기능 >​
- 감정 분석 정보를 이용하여 음악을 추천​
- 음악 검색 : 쿼리 기능을 이용하여 트랙, 플레이리스트를 검색​
- 음악 추가 : 검색된 트랙, 플레이리스트를 감정 카테고리 플레이리스트에 추가​
- 음악 재생 제어 : 재생, 일시 정지, 다음 곡/이전 곡 넘기기 기능
<br>

< 다루는 정보 >​
- AWS Rekognition 감정 분석 정보​
- user 테이블 내 user_id 정보​
- emotion 테이블 내 emotion_name에 할당된 태그 정보​
- Spotify 내에서 검색 가능한 태그를 미리 설정​
- Spotify 내 사용자의 트랙, 플레이리스트 정보
<br>
...
< 시점 >​
- 감정 분석 결과 기록: 매번 하나의 음악 재생이 끝날 시​
- 집중도 레벨 기록: 실시간
